    ...
    <div id="layoutSidenav_content">
        <main>
            ...
            <h3 id="'.$user_type.'"> </h3>
                ...
                $sql='SELECT * FROM `donor` where `d_no`='.$d_no.'';
                $result=mysqli_query($con, $sql);
                $row=mysqli_fetch_assoc($result);
                $user_type=$_SESSION['user'];
                echo'<h3 id="usertype"> </h3>
                <h3 id="'.$user_type.'"> </h3>';
                echo'<h4 id="edit_profile">Information of '.$row['d_name'].' </h4>
                
                <div id="'.$d_no.'">
                <p><label for="">Patient Name: </label> <span><strong>'.$row['d_name'].'</strong></span></p>
                <p><label for="">Blood Group: </label> <span><strong>'.$row['d_blood_group'].'</strong></span></p>
                <p><label for="">Adress: </label> <span><strong id="privious_address">'.$row['d_address'].'</strong></span></p>
                <p><label for="">City: </label> <span><strong id="privious_city">'.$row['d_city'].'</strong></span></p>
                <p><label for="">Phone Number: </label> <span><strong id="privious_phn_no">'.$row['d_phn_no'].'</strong></span></p>

                <p><label for="">Updated Phone Number:</label><input type="number" name="update_phn_no" id="update_phn_no" value="'.$row['d_phn_no'].'">
                <button class="btn btn-primary px-4 mx-4" id="update_phn_no_button" name="update_phn_no_button">Edit</button></p>

                <label for="">Updated Address:</label><input type="text" name="update_address" id="update_address" value="'.$row['d_address'].'">
